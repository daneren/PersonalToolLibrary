...
# 要计算平均值的指标
metric_key = "timing_per_token_ms/gen"
...
print(f"--- 计算指标: {metric_key} 在 Step {START_STEP} 到 {END_STEP} 范围内的平均值 ---")
mean1 = calculate_mean_metric_in_range(run1_path, metric_key, START_STEP, END_STEP)
mean2 = calculate_mean_metric_in_range(run2_path, metric_key, START_STEP, END_STEP)

print(f"mean1: {mean1}")
print(f"mean2: {mean2}")

print(f"mean2 / mean1: {mean2 / mean1:.2f}")
